...
// Body: { instructionId: string, reason: string }
export async function POST(req: NextRequest) {
  try {
    const { instructionId, reason } = await req.json();
    if (!instructionId || !reason) {
      return NextResponse.json({ error: 'Missing instructionId or reason' }, { status: 400 });
    }
