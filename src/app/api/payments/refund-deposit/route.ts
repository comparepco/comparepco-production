...
// Body: { instructionId: string, refundAmount: number, reason?: string }
export async function POST(req: NextRequest) {
  try {
    const { instructionId, refundAmount, reason } = await req.json();
    if (!instructionId) {
    ...
    // Fetch related entities (booking, driver, partner, etc.)
    ...
    const { error: historyError } = await supabaseAdmin
      ...
      .insert({
        ...
        description: `Deposit refund of £${refundAmount} issued${reason ? `: ${reason}` : ''}`,
        created_at: new Date().toISOString()
      });
    ...
    const { error: notificationError } = await supabaseAdmin
      ...
      .insert({
        ...
        message: `Your deposit (£${refundAmount}) has been refunded by the partner.${reason ? ` Reason: ${reason}` : ''}`,
        data: {
          instruction_id: instructionId,
          booking_id: instruction.booking_id,
          amount: refundAmount,
